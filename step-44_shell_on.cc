...
// iterations for the Newton-Raphson nonlinear solver.
    ...
    void NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry("Tolerance displacement", "1.0e-2",
                          Patterns::Double(0.0),
...
//                                   Point<dim>(2.0, 1.0, 1.0),
//                                   true);
    const double inner_radius = 2.6,outer_radius = 4;
    const Point<dim> center(0,0,0);
    GridGenerator::half_hyper_shell (triangulation, center, inner_radius, outer_radius,0);

    GridTools::scale(parameters.scale, triangulation);
//    std::vector b_i= triangulation.get_boundary_indicators();
//    std::cout<<"b_indicator1 ="<<b_i(0);
//    std::cout<<"b_indicator2 ="<<b_i(1);
        	...
        	if (cell->face(face)->at_boundary()){
        	    const Point<dim> face_center = cell->face(face)->center(); 
           	    if (std::sqrt(face_center[0]*face_center[0] +
                     ...
                     && face_center[0] >= 0.1*parameters.scale)
    ...
    triangulation.set_boundary(1,boundary_description1);  
    static const HyperShellBoundary<dim> boundary_description2(center);
    triangulation.set_boundary(2,boundary_description2);      
  ...
  Solid<dim>::solve_nonlinear_timestep(BlockVector<double> & solution_delta)
  {
    ...
    for (; newton_iteration < parameters.max_iterations_NR;
         ++newton_iteration)
      {
        ...
        if (newton_iteration > 0 && error_update_norm.u <= 0.01)//parameters.tol_u
