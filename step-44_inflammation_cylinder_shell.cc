...
//***********************************************************************************************
    ...
    rd_system_rhs.reinit (rd_dof_handler.n_dofs());
...
//*************************************************************************************************
  ...
  {
    SolverControl           solver_control (1000, 1e-8*rd_system_rhs.l2_norm());
    SolverCG<>              cg (solver_control);

    cg.solve (matrix_u, solution_u, rd_system_rhs,
...
//         time+=time_step, ++timestep_number)
    ...
    VectorTools::create_right_hand_side (rd_dof_handler, QGauss<dim>(2),
                                         rhs_function, tmp);
    forcing_terms = tmp;
    forcing_terms *= theta * time_step;

    rhs_function.set_time (time.current()-time_step);
    VectorTools::create_right_hand_side (rd_dof_handler, QGauss<dim>(2),
                                         rhs_function, tmp);

    forcing_terms.add ((1-theta) * time_step, tmp);

    rd_system_rhs.add (theta * time_step, forcing_terms);




    laplace_matrix.vmult (rd_system_rhs, solution_u);
    rd_system_rhs *= -0.05*0.001*0.001*theta * time_step;

    mass_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs += tmp;

    laplace_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs.add (-0.05*0.001*0.001*time_step * (1-theta), tmp);

    rd_system_rhs += forcing_terms;
