...
//    std::cout<<"b_indicator2 ="<<b_i(1);
    ...
    for (; cell != endc; ++cell)
      for (unsigned int face = 0;
        ...
        if ((cell->face(face)->boundary_indicator() == 2 || cell->face(face)->boundary_indicator() == 1)
            && cell->face(face)->center()[1]*cell->face(face)->center()[1]
            +  cell->face(face)->center()[2]*cell->face(face)->center()[2] < 0.25*parameters.scale*parameters.scale)
               cell->face(face)->set_boundary_indicator(3);
      }
      
    static const CylinderBoundary<dim> cylinder_boundary(parameters.scale,0);
