...
namespace Step44
{
                                     ...
                                     const unsigned int component) const
  {
    Assert (component == 0, ExcInternalError());
     if (p[0]*p[0] + p[1]*p[1]>0.7*0.7*0.001*0.001 )return 0.0;
     else if (p[0]*p[0] + p[1]*p[1]<=0.7*0.7*0.001*0.001 && p[0]*p[0] + p[1]*p[1]>0.5*0.5*0.001*0.001)
         return -sin((sqrt(p[0]*p[0] + p[1]*p[1])-0.6*0.001)/0.1/0.001*numbers::PI/2)*0.5+0.5;
     else return 1.0;
  }
...
double collagen_density[100][100];
				       ...
				       Vector<double>   &values) const 
{
  ...
  values(0) = collagen_density[(int)(p(1)*1000*100)][(int)(p(0)*1000*100)];
...
// results given in the literature.
    ...
    void Geometry::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry");
      {
        global_refinement = 3;//prm.get_integer("Global refinement");
    ...
    for (; cell != endc; ++cell)
      for (unsigned int face = 0;
        ...
        if (cell->face(face)->at_boundary() == true
          ...
          if (cell->face(face)->center()[0] < 0.75 * parameters.scale
              &&
              cell->face(face)->center()[0] > 0.25 * parameters.scale
              &&
              cell->face(face)->center()[1] > 0.25 * parameters.scale
              &&
              cell->face(face)->center()[1] < 0.75 * parameters.scale)
...
//*************************************************************************************************************** 
    ...
    for (i=0;i<100;i++){
        for (j=0;j<100;j++){
            ...
            if(sqrt(pow(i-50,2.0)+pow(j-50,2.0))<25)collagen_density[i][j] = 0.5;
        }
    }
          ...
          && cell->face(face)->boundary_indicator() == 6)
        {
          ...
          for (unsigned int f_q_point = 0; f_q_point < n_q_points_f;
               ++f_q_point)
            {
              ...
              double         pressure;
              if(time_ramp<0.5) pressure  = p0 * parameters.p_p0 * time_ramp;
              else pressure = p0 * parameters.p_p0 * (1-time_ramp);
