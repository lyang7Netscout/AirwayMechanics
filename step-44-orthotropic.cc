...
//          F[2][2] = 1;
      ...
      void update_material_data(const Tensor<2, dim> & F_par,
                                const double p_tilde_in,
                                const double J_tilde_in)
        {
          det_F = determinant(F_par);
          F = F_par;
      ...
      Tensor<2, dim> get_tau()
        {
          return get_tau_iso1() + get_tau_iso4() + get_tau_vol();
        }
      ...
      Tensor<4, dim> get_Jc() const
        {
          Tensor<4, dim> tmp1 = get_Jc_iso1();
          Tensor<4, dim> tmp2 = get_Jc_iso4();
          Tensor<4, dim> tmp6 = get_Jc_iso6();          
          Tensor<4, dim> tmp3 = get_Jc_vol();
          return tmp1+tmp2+tmp3+tmp6;
        }
    ...
    protected:
      ...
      SymmetricTensor<2, dim> get_tau_iso1() const
        {
          return StandardTensors<dim>::dev_P * get_tau_bar1();
        }
      Tensor<2, dim> get_tau_iso4() const
        {
          Tensor<4,dim> tmp1 = StandardTensors<dim>::dev_P; 
          Tensor<2,dim> tmp2 = get_tau_bar4();
          Tensor<2,dim> tmp = perform_double_contraction(tmp1,tmp2); 
          return  tmp;
        }
      Tensor<2, dim> get_tau_iso6() const
        {
          Tensor<4,dim> tmp1 = StandardTensors<dim>::dev_P;
          Tensor<2,dim> tmp2 = get_tau_bar6();
          Tensor<2,dim> tmp = perform_double_contraction(tmp1,tmp2);
          return tmp;
        }
      ...
      SymmetricTensor<2, dim> get_tau_bar1() const
        {
          return 2.0 * c_1 * b_bar;
        }
      Tensor<2, dim> get_tau_bar4() const
        { 
          Tensor<1, dim> tmp,tmp2;
          Tensor<2, dim> F_trans = transpose(F);
          contract(tmp, F, M);
          contract(tmp2, F_trans, tmp);
          double I4 = contract(M, tmp2);
          Tensor<2, dim> M_tensormul_M;
          outer_product(M_tensormul_M,M,M);
          Tensor<2, dim> b_bar_copy = b_bar;
          Tensor<2, dim> tmp3 = b_bar_copy*M_tensormul_M;
          return 2*k1*exp(k2*pow(I4-1,2))*(I4-1)*tmp3;
        }
        
      Tensor<2, dim> get_tau_bar6() const
        { 
          Tensor<1, dim> tmp,tmp2;
          Tensor<2, dim> F_trans = transpose(F);
          contract(tmp, F, M2);
          contract(tmp2, F_trans, tmp);
          double I6 = contract(M2, tmp2);
          Tensor<2, dim> M2_tensormul_M2;
          outer_product(M2_tensormul_M2,M2,M2);
          Tensor<2, dim> b_bar_copy = b_bar;
          Tensor<2, dim> tmp3 = b_bar_copy*M2_tensormul_M2;
          return 2*k1*exp(k2*pow(I6-1,2))*(I6-1)*tmp3;
        }
      ...
      SymmetricTensor<4, dim> get_Jc_iso1() const
        {
          const SymmetricTensor<2, dim> tau_bar1 = get_tau_bar1();
          const SymmetricTensor<2, dim> tau_iso1 = get_tau_iso1();
          const SymmetricTensor<4, dim> tau_iso1_x_I
            = outer_product(tau_iso1,
                            StandardTensors<dim>::I);
          const SymmetricTensor<4, dim> I_x_tau_iso1
            = outer_product(StandardTensors<dim>::I,
                            tau_iso1);
          const SymmetricTensor<4, dim> c_bar1 = get_c_bar1();

          return (2.0 / 3.0) * trace(tau_bar1)
            * StandardTensors<dim>::dev_P
            - (2.0 / 3.0) * (tau_iso1_x_I + I_x_tau_iso1)
            + StandardTensors<dim>::dev_P * c_bar1
            * StandardTensors<dim>::dev_P;
        }
        
      Tensor<4, dim> get_Jc_iso4() const
        {
          const Tensor<2, dim> tau_bar4 = get_tau_bar4();
          const Tensor<2, dim> tau_iso4 = get_tau_iso4();
          Tensor<2, dim> any_I = StandardTensors<dim>::I;
          const Tensor<4, dim> tau_iso4_x_I
            = outer_product(tau_iso4,any_I);
          const Tensor<4, dim> I_x_tau_iso4
            = outer_product(any_I,tau_iso4);
          Tensor<4, dim> c_bar4 = get_c_bar4();

          Tensor<4, dim> any_dev_P = StandardTensors<dim>::dev_P;
          Tensor<4, dim> first_contraction = perform_double_contraction(c_bar4, any_dev_P);
          Tensor<4, dim> second_contraction = perform_double_contraction(any_dev_P, first_contraction);
      ...
      Tensor<4, dim> get_Jc_iso6() const
        {
          const Tensor<2, dim> tau_bar6 = get_tau_bar6();
          const Tensor<2, dim> tau_iso6 = get_tau_iso6();
          Tensor<2, dim> any_I = StandardTensors<dim>::I;
          const Tensor<4, dim> tau_iso6_x_I
            = outer_product(tau_iso6,any_I);
          const Tensor<4, dim> I_x_tau_iso6
            = outer_product(any_I,tau_iso6);
          Tensor<4, dim> c_bar6 = get_c_bar6();

          Tensor<4, dim> any_dev_P = StandardTensors<dim>::dev_P;
          Tensor<4, dim> first_contraction = perform_double_contraction(c_bar6, any_dev_P);
          Tensor<4, dim> second_contraction = perform_double_contraction(any_dev_P, first_contraction);
          return (2.0 / 3.0) * trace(tau_bar6)
            * any_dev_P
            - (2.0 / 3.0) * (tau_iso6_x_I + I_x_tau_iso6)
            + second_contraction;
        }
      ...
      SymmetricTensor<4, dim> get_c_bar1() const
        {
          ...
          Tensor<2, dim> dI4_dC_copy;
          outer_product(dI4_dC_copy,M,M);//see Holzapfel paper D4
          SymmetricTensor<2, dim> dI4_dC = dI4_dC_copy;
          double dPhi_iso_dI1;
      ...
      SymmetricTensor<4, dim> get_c_bar4() const
        {
          Tensor<1, dim> tmp,tmp2;
          Tensor<2, dim> F_trans = transpose(F);
          contract(tmp, F, M);
          ...
          double I4 = contract(M, tmp2);
          double ddPhi_iso_dI1_dI4=0;
          double ddPhi_iso_dI4_dI4=k1*exp(k2*pow(I4-1,2))*(2*k2*pow(I4-1,2)+1);
          SymmetricTensor<2, dim> dI1_dC=StandardTensors<dim>::I;
          Tensor<2, dim> dI4_dC_copy;
          outer_product(dI4_dC_copy,M,M);
          SymmetricTensor<2, dim> dI4_dC=dI4_dC_copy;
          double dPhi_iso_dI4=k1*exp(k2*pow(I4-1,2));
          SymmetricTensor<4, dim> ddI4_dC_dC;
          return 4*(ddPhi_iso_dI1_dI4*outer_product(dI4_dC,dI1_dC)//==0
                   +ddPhi_iso_dI4_dI4*outer_product(dI4_dC,dI4_dC)//!=0
                   +2*dPhi_iso_dI4*ddI4_dC_dC);//==0
        }
        
        SymmetricTensor<4, dim> get_c_bar6() const
        {
          Tensor<1, dim> tmp,tmp2;
          Tensor<2, dim> F_trans = transpose(F);
          contract(tmp, F, M2);
          contract(tmp2, F_trans, tmp);
          double I6 = contract(M2, tmp2);
          double ddPhi_iso_dI1_dI6=0;
          double ddPhi_iso_dI6_dI6=k1*exp(k2*pow(I6-1,2))*(2*k2*pow(I6-1,2)+1);
          SymmetricTensor<2, dim> dI1_dC=StandardTensors<dim>::I;
          Tensor<2, dim> dI6_dC_copy;
          outer_product(dI6_dC_copy,M,M);
          SymmetricTensor<2, dim> dI6_dC=dI6_dC_copy;
          double dPhi_iso_dI6=k1*exp(k2*pow(I6-1,2));
          SymmetricTensor<4, dim> ddI6_dC_dC;
          return 4*(ddPhi_iso_dI1_dI6*outer_product(dI6_dC,dI1_dC)//==0
                   +ddPhi_iso_dI6_dI6*outer_product(dI6_dC,dI6_dC)//!=0
                   +2*dPhi_iso_dI6*ddI6_dC_dC);//==0
        }
  };
...
// the quadrature points.
  ...
  class PointHistory
  {
    public:
      ...
      virtual ~PointHistory()
        {
          ...
          material = NULL;
				       ...
				       // values.
      void setup_lqp (const Parameters::AllParameters & parameters, Tensor<1,dim> M_par, Tensor<1,dim> M2_par)
        {
      ...
      const Tensor<4, dim>& get_Jc() const
        {
          return Jc;
        }
...
// for comparison:
  ...
  {        
    ...
    GridGenerator::cylinder_shell (triangulation, 2, inner_radius, outer_radius);
    ...
    for (; cell != endc; ++cell)
      for (unsigned int face = 0;face < GeometryInfo<dim>::faces_per_cell; ++face){
          	if (cell->face(face)->at_boundary())
        	  {
        	    ...
        	    if (face_center[2] == 0)
        	    ...
        	    else if (face_center[2] == 2*parameters.scale)
        	      cell->face(face)->set_boundary_indicator (1);
                           ...
                           && face_center[2] > 0.5*parameters.scale && face_center[2] <1.5*parameters.scale)
    ...
    for (typename Triangulation<dim>::active_cell_iterator cell =
           triangulation.begin_active(); cell != triangulation.end(); ++cell)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point){
            ...
            M_par[0] = -cos(3.141592654/3)*points[q_point](1)/sqrt(points[q_point](1)*points[q_point](1)+points[q_point](0)*points[q_point](0));
            M_par[1] = cos(3.141592654/3)*points[q_point](0)/sqrt(points[q_point](1)*points[q_point](1)+points[q_point](0)*points[q_point](0));
            M_par[2] = sin(3.141592654/2);   
            M2_par[0] = cos(3.141592654/3)*points[q_point](1)/sqrt(points[q_point](1)*points[q_point](1)+points[q_point](0)*points[q_point](0));
            M2_par[1] = -cos(3.141592654/3)*points[q_point](0)/sqrt(points[q_point](1)*points[q_point](1)+points[q_point](0)*points[q_point](0));
            M2_par[2] = sin(3.141592654/2);     
            //std::cout<<M_par[0]<<" "<<M_par[1]<<std::endl;    
            lqph[q_point].setup_lqp(parameters,M_par,M2_par);
        }
      }
  }
    ...
    for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
      {
        ...
        Tensor<4, dim> Jc       = lqph[q_point].get_Jc();
                ...
                if ((i_group == j_group) && (i_group == u_dof))
                  {
                    ...
                    Tensor<2,dim> tmp2 = perform_double_contraction(tmp, tmp_Jc);
                    Tensor<2,dim> tmp3 = symm_grad_Nx[j];
                    double tmp4 = perform_double_contraction(tmp2, tmp3);
...
//                                              * symm_grad_Nx[j] * JxW;
                  ...
                  Assert((i_group <= J_dof) && (j_group <= J_dof),
                         ExcInternalError());
                //if(i==30 && j==20)std::cout<<data.cell_matrix(i, j)<<std::endl;
              }
          }
      }
...
int main (int argc, char *argv[])
{
  ...
  std::cout<<"tensor conversed from symmetric tensor..."<<std::endl;
  ...
  Tensor<2,3,double> tensor_contraction1 = perform_double_contraction(tensor4, tensorA2);
  double tensor_contraction2 = perform_double_contraction(tensorA2,tensor_contraction1);
...
//  std::cout<<tensor_contraction[2][0][0][0]<<" "<<tensor_contraction[2][0][0][1]<<" "<<tensor_contraction[2][0][0][2]<<std::endl;
//  std::cout<<tensor_contraction[2][0][1][0]<<" "<<tensor_contraction[2][0][1][1]<<" "<<tensor_contraction[2][0][1][2]<<std::endl;
//  std::cout<<tensor_contraction[2][0][2][0]<<" "<<tensor_contraction[2][0][2][1]<<" "<<tensor_contraction[2][0][2][2]<<std::endl;
