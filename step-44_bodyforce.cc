...
// and precomputed values vector:
  ...
  struct Solid<dim>::ScratchData_RHS
  {
      ...
      std::vector<std::vector<double> >                   Nx;
//************************************self added**********************************
      std::vector<std::vector<Tensor<1,dim> > >           Nx_u;
      std::vector<std::vector<SymmetricTensor<2, dim> > > symm_grad_Nx;
    ...
    for (; cell != endc; ++cell)
      for (unsigned int face = 0;
        ...
        if (cell->face(face)->at_boundary() == true
            &&
            cell->face(face)->center()[2] == 2.0 * parameters.scale)
          if (cell->face(face)->center()[0] < 0.75*2 * parameters.scale
              &&
              cell->face(face)->center()[0] > 0.25*2 * parameters.scale
              &&
              cell->face(face)->center()[1] > 0.25*2 * parameters.scale
              &&
              cell->face(face)->center()[1] < 0.75*2 * parameters.scale)
          ...
          if (k_group == u_dof){
            scratch.symm_grad_Nx[q_point][k]
              = symmetrize(scratch.fe_values_ref[u_fe].gradient(k, q_point)
                           * F_inv);
            scratch.Nx_u[q_point][k] = scratch.fe_values_ref[u_fe].value(k,q_point);
          }
          else if (k_group == p_dof)
            scratch.Nx[q_point][k] = scratch.fe_values_ref[p_fe].value(k,
                                                                       q_point);
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            ...
            if (i_group == u_dof)
              data.cell_rhs(i) -= (symm_grad_Nx[i] * tau) * JxW 
                                   + (Nx_u[i][0] * 100000 ) * JxW;
//**********************************self added, used to describe contribution from body force***********************
