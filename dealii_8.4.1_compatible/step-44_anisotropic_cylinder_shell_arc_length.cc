...
// iterations for the Newton-Raphson nonlinear solver.
    ...
    void NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry("Tolerance residual load", "1.0e-3",
                          Patterns::Double(0.0),
                          "Residual load tolerance");

        prm.declare_entry("Arc length", "3.0",
                          Patterns::Double(0.0),
                          "Arc length");
      }
      prm.leave_subsection();
    ...
    void NonlinearSolver::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        tol_res_load = prm.get_double("Tolerance residual load");
        arc_length0 = prm.get_double("Arc length");
      }
      prm.leave_subsection();
    }
...
// the quadrature points.
  ...
  public:
    ...
    void setup_lqp (const Parameters::AllParameters &parameters, Tensor<1,dim> M_par)
    {
...
// all the work to private functions of this class:
  ...
  private:
    ...
    void
    assemble_system_rhs(bool Q_tag);
    ...
    while (time.current() < time.end())
      {
        ...
        // \varDelta \mathbf{\Xi}$...
        solve_nonlinear_timestep(solution_delta, parameters.arc_length0);
...
// reset the error storage objects and print solver header.
  template <int dim>
  void
  Solid<dim>::solve_nonlinear_timestep(BlockVector<double> & solution_delta, double arc_length0)
  {
    ...
    for (; newton_iteration < parameters.max_iterations_NR;
         ++newton_iteration)
      {
...

        {
            ...
            if (b*b - a*c > 0){
                std::pair<double, double> Delta_lambda_pair = std::make_pair((-b-sqrt(b*b-a*c))/a, (-b+sqrt(b*b-a*c))/a);
                std::pair<BlockVector<double>, BlockVector<double> > Delta_u_pair = std::make_pair(Delta_uR + Delta_lambda_pair.first*Delta_uQ, 
                                                                             Delta_uR + Delta_lambda_pair.second*Delta_uQ);
                std::pair<BlockVector<double>, BlockVector<double> > solution_i_pair = std::make_pair(solution_delta + Delta_u_pair.first, 
                                                                                solution_delta + Delta_u_pair.second);
                
                std::pair<BlockVector<double>, BlockVector<double> > solution_update_pair = std::make_pair(solution_i_pair.first.block(u_dof) - solution_n.block(u_dof), 
                                                                                     sulotion_i_pair.second.block(u_dof) - solution_n.block(u_dof));
                std::pair<double, double> cosines = std::make_pair(solution_n*solution_update.first/abs(solution_n*solution_update.first), 
