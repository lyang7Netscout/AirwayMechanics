...
// current state.
  ...
  public:
    ...
    {
      Tensor<2, dim> P_vol = get_P_vol();
      Tensor<2, dim> tau_vol = get_tau_vol();
      assert(tau_vol == P_vol * transpose(F));

      Tensor<2, dim> P_iso = get_P_iso();
      Tensor<2, dim> tau_iso = get_tau_iso();
      assert(tau_iso == P_iso * transpose(F));

      Tensor<4, dim> A = get_A();
      Tensor<4, dim> Jc = get_Jc();
      assert(Jc == F * A * transpose(F) );
    }
  protected:
    ...
    {
      SymmetricTensor<2, dim> tau_bar = 2.0 * c_1 * b_bar;
      return StandardTensors<dim>::dev_P * tau_bar;
    }
    ...
    Tensor<2, dim> get_P_vol() const
    {
      Tensor<2, dim> F_inv = invert(F);
      return p_tilde * det_F * transpose(F_inv);
    }
...
// $J\mathfrak{c}$ for the quadrature points.
  ...
  public:
    ...
    const Tensor<2, dim> &get_P() const
    {
      return P;
    }
    ...
    const Tensor<4, dim> &get_A() const
    {
      return A;
    }
    ...
    for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
      {
        ...
        const Tensor<4, dim> A           = lqph[q_point].get_A();
            ...
            for (unsigned int j = 0; j <= i; ++j)
              {
                ...
                if ((i_group == j_group) && (i_group == u_dof))
                  {
                    Tensor<2, dim> tmp = double_contract<0,0,1,1>(Grad_Nx[i], A);
                    data.cell_matrix(i, j) += double_contract<0,0,1,1>(tmp, Grad_Nx[j]) // The material contribution:
                                               * JxW;
                  }
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            ...
            if (i_group == u_dof)
              data.cell_rhs(i) -= double_contract<0,0,1,1>(Grad_Nx[i], P) * JxW;
