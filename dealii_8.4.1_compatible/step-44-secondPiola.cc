...
// current state.
  ...
  public:
                              ...
                              const double J_tilde_in)
    {
      ...
      Cinv = symmetrize(invert(F) * invert(transpose(F)));
      p_tilde = p_tilde_in;
      J_tilde = J_tilde_in;
    ...
    SymmetricTensor<2, dim> get_S()
    {
      return get_S_iso() + get_S_vol();
    }
    ...
    {
      return get_Cmaterial_vol() + get_Cmaterial_iso();
    }
    ...
    void testConsistency()
    {
      ...
      Tensor<2, dim> temp = F * S_vol * transpose(F);
      assert(abs(tau_vol[0][0] - temp[0][0]) < 1e-6);
      assert(abs(tau_vol[0][1] - temp[0][1]) < 1e-6);
      assert(abs(tau_vol[0][2] - temp[0][2]) < 1e-6);
      assert(abs(tau_vol[1][0] - temp[1][0]) < 1e-6);
      assert(abs(tau_vol[1][1] - temp[1][1]) < 1e-6);
      assert(abs(tau_vol[1][2] - temp[1][2]) < 1e-6);
      assert(abs(tau_vol[2][0] - temp[2][0]) < 1e-6);
      assert(abs(tau_vol[2][1] - temp[2][1]) < 1e-6);
      assert(abs(tau_vol[2][2] - temp[2][2]) < 1e-6);

      Tensor<2, dim> S_iso = get_S_iso();
      Tensor<2, dim> tau_iso = get_tau_iso();
      Tensor<2, dim> temp2 = F * S_iso * transpose(F);
      assert(abs(tau_iso[0][0] - temp2[0][0]) < 1e-6);
      assert(abs(tau_iso[0][1] - temp2[0][1]) < 1e-6);
      assert(abs(tau_iso[0][2] - temp2[0][2]) < 1e-6);
      assert(abs(tau_iso[1][0] - temp2[1][0]) < 1e-6);
      assert(abs(tau_iso[1][1] - temp2[1][1]) < 1e-6);
      assert(abs(tau_iso[1][2] - temp2[1][2]) < 1e-6);
      assert(abs(tau_iso[2][0] - temp2[2][0]) < 1e-6);
      assert(abs(tau_iso[2][1] - temp2[2][1]) < 1e-6);
      assert(abs(tau_iso[2][2] - temp2[2][2]) < 1e-6);
      

      SymmetricTensor<4, dim> Cvol = get_Cmaterial_vol();
      SymmetricTensor<4, dim> Jc_vol = get_Jc_vol();
      SymmetricTensor<4, dim> Ciso = get_Cmaterial_iso();
      SymmetricTensor<4, dim> Jc_iso = get_Jc_iso();
                  ...
                  for(int l = 0;l < dim; l++){
                      ...
                      for(int a = 0;a < 3; a++){
                          for(int b = 0;b < 3; b++){
                              for(int c = 0;c < 3; c++){
                                  for(int d = 0;d < 3; d++){
                                      temp += F[i][a] * F[j][b] * F[k][c] * F[l][d] * Cvol[a][b][c][d];
                                      temp2+= F[i][a] * F[j][b] * F[k][c] * F[l][d] * Ciso[a][b][c][d];
                                  }
                              }
                          }
                      }
  ...
  protected:
    ...
    {
      return p_tilde * det_F * Cinv;
    }

    SymmetricTensor<2, dim> get_S_iso() const
    {
      Tensor<2, dim> temp = StandardTensors<dim>::dev_P * C;
      return 2.0 * c_1 * std::pow(det_F, -2.0/3.0) * 
             symmetrize( temp * Tensor<2, dim>(Cinv) );
    }
    ...
    SymmetricTensor<4, dim> get_Jc_iso() const
    {
      ...
      SymmetricTensor<4, dim> result1 =  (2.0 / 3.0) * trace(tau_bar) * StandardTensors<dim>::dev_P
      ...
      const SymmetricTensor<4, dim> I_x_I = outer_product(StandardTensors<dim>::I, StandardTensors<dim>::I);
      const SymmetricTensor<4, dim> b_x_I = outer_product(b, StandardTensors<dim>::I);
      const SymmetricTensor<4, dim> I_x_b = outer_product(StandardTensors<dim>::I, b);
      
      SymmetricTensor<4, dim> result2 = 4.0 / 3.0 * c_1 * std::pow(det_F, -2.0/3.0) * (
                          1.0 / 3.0 * trace(b) * I_x_I - b_x_I - I_x_b + trace(b)*StandardTensors<dim>::II);
      assert( abs(result1[1][1][2][2] - result2[1][1][2][2]) < 1e-6);
      assert( abs(result1[1][1][0][2] - result2[1][1][0][2]) < 1e-6);
      assert( abs(result1[1][2][2][2] - result2[1][2][2][2]) < 1e-6);
      assert( abs(result1[1][0][2][1] - result2[1][0][2][1]) < 1e-6);
      assert( abs(result1[0][1][2][2] - result2[0][1][2][2]) < 1e-6);
    ...
    SymmetricTensor<4, dim> get_Cmaterial_vol() const
    {
      ...
      for(int i = 0; i < dim; i++){
          for(int j = 0; j < dim; j++){
              for(int k = 0; k < dim; k++){
                  for(int l = 0; l < dim; l++){
                      //to validate relation between II_sym and II_Cinv
                      double temp=0.0;
                      for(int a = 0; a < dim; a++){
                          for(int b = 0; b < dim; b++){
                              temp += Cinv[k][b] * (StandardTensors<dim>::II)[i][j][b][a] * Cinv[a][l];
                          }
                      }
                      assert(temp == II_Cinv[i][j][k][l]);

                      //to validate push forward formula of elasticity tensor
                      double temp2 = 0.0;
                      for(int a = 0; a < dim; a++){
                          for(int b = 0; b < dim; b++){
                              for(int c = 0; c < dim; c++){
                                  for(int d = 0; d < dim; d++){
                                      temp2 += F[i][a] * F[j][b] * F[k][c] * F[l][d] * II_Cinv[a][b][c][d];
                                  }
                              }
                          }
                      }
                      assert(abs(temp2 - (StandardTensors<dim>::II)[i][j][k][l]) < 1e-6);
                  }
              }
          }
      }
      ...
      for(int i = 0; i < dim; i++){
          for(int j = 0; j < dim; j++){
              for(int k = 0; k < dim; k++){
                  for(int l = 0; l < dim; l++){
                      //to validate relation between I_x_I and Cinv_x_Cinv
                      double temp  = 0.0;
                      for(int a = 0; a < dim; a++){
                          for(int b = 0; b < dim; b++){
                              temp += Cinv[i][a] * StandardTensors<dim>::IxI[a][j][k][b] * Cinv[b][l];
                          }
                      }
                      assert(temp == Cinv_x_Cinv[i][j][k][l]);
                      ...
                      for(int a = 0; a < dim; a++){
                          for(int b = 0; b < dim; b++){
                              for(int c = 0; c < dim; c++){
                                  for(int d = 0; d < dim; d++){
                                      temp2 += F[i][a] * F[j][b] * F[k][c] * F[l][d] * Cinv_x_Cinv[a][b][c][d];
                                  }
                              }
                          }
                      }
                      assert(abs(temp2 - StandardTensors<dim>::IxI[i][j][k][l]) < 1e-6);
                  }
              }
          }
      }
      ...
      return p_tilde * det_F * (Cinv_x_Cinv - 2*II_Cinv); 
    }

    SymmetricTensor<4, dim> get_Cmaterial_iso() const
    {
      SymmetricTensor<4, dim> Cinv_x_Cinv = outer_product(Cinv, Cinv);
      ...
      return (4.0 / 3.0) * c_1 * std::pow(det_F, -2.0 / 3.0) * (
