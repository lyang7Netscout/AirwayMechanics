...
namespace Step44
{
                                     ...
                                     const unsigned int component) const
  {
    Assert (component == 0, ExcInternalError());
     if (p[1]*p[1] + p[2]*p[2]>0.7*0.7*0.001*0.001 )return 0.0;
     else if (p[1]*p[1] + p[2]*p[2]<=0.7*0.7*0.001*0.001 && p[1]*p[1] + p[2]*p[2]>0.5*0.5*0.001*0.001)
         return -sin((sqrt(p[1]*p[1] + p[2]*p[2])-0.6*0.001)/0.1/0.001*numbers::PI/2)*0.5+0.5;
     else return 1.0;
  }
...
//                cde_values[p](0) = 1.0-0.5*TNF_value(0);
//          }
        }
        double c_density;
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point){
...
//         time+=time_step, ++timestep_number)
    ...
    rd_system_rhs *= -0.05*0.001*theta * time_step;

    mass_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs += tmp;

    laplace_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs.add (-0.05*0.001*time_step * (1-theta), tmp);
