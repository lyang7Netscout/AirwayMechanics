...
namespace Step44
{
                                     ...
                                     const unsigned int component) const
  {
    Assert (component == 0, ExcInternalError());
     if (p[1]*p[1] + p[0]*p[0]>0.7*0.7*0.001*0.001 && p[2]>0)return 1.0;
     //else if (p[1]*p[1] + p[0]*p[0]<=0.7*0.7*0.001*0.001 && p[1]*p[1] + p[0]*p[0]>0.5*0.5*0.001*0.001)
     //    return -sin((sqrt(p[1]*p[1] + p[0]*p[0])-0.6*0.001)/0.1/0.001*numbers::PI/2)*0.5+0.5;
     else return 0.0;
...
// iterations for the Newton-Raphson nonlinear solver.
    ...
    void NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry("Tolerance displacement", "1.0e-4",
                          Patterns::Double(0.0),
...
// for comparison:
  ...
  {    
    const double inner_radius = 1.3,outer_radius = 2;
    const Point<dim> center(0,0,0);
    GridGenerator::half_hyper_shell (triangulation, center, inner_radius, outer_radius,0);
        	...
        	if (cell->face(face)->at_boundary()){
        	    const Point<dim> face_center = cell->face(face)->center(); 
           	    if (std::sqrt(face_center[0]*face_center[0] +
        		     ...
        		     <(inner_radius + outer_radius)*parameters.scale / 2
                     && face_center[0] >= 1.0*parameters.scale)
        	      cell->face(face)->set_boundary_indicator (1); 
                if (face_center[0] == 0)cell->face(face)->set_boundary_indicator(2);      
     	    }
      }
      
    static const HyperShellBoundary<dim> boundary_description(center);
    triangulation.set_boundary(0,boundary_description);
    static const HyperShellBoundary<dim> boundary_description1(center);
    triangulation.set_boundary(1,boundary_description);  
    static const HyperShellBoundary<dim> boundary_description2(center);
  ...
  Solid<dim>::solve_nonlinear_timestep(BlockVector<double> & solution_delta)
  {
    ...
    for (; newton_iteration < parameters.max_iterations_NR;
         ++newton_iteration)
      {
        ...
        if (newton_iteration > 0 && error_update_norm.u <= 0.0001)//parameters.tol_u
...
//         time+=time_step, ++timestep_number)
    ...
    rd_system_rhs *= -0.05*0.0001*theta * time_step;

    mass_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs += tmp;

    laplace_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs.add (-0.05*0.0001*time_step * (1-theta), tmp);
