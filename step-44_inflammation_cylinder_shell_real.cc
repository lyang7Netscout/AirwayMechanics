...
namespace Step44
{
                                     ...
                                     const unsigned int component) const
  {
    Assert (component == 0, ExcInternalError());
     if (p[1]*p[1] + (p[2]-0.003)*(p[2]-0.003)>0.7*0.7*0.001*0.001 )return 0.0;
     else if (p[1]*p[1] + (p[2]-0.003)*(p[2]-0.003)<=0.7*0.7*0.001*0.001 && p[1]*p[1] + (p[2]-0.003)*(p[2]-0.003)>0.5*0.5*0.001*0.001)
         return -sin((sqrt(p[1]*p[1] + (p[2]-0.003)*(p[2]-0.003))-0.6*0.001)/0.1/0.001*numbers::PI/2)*1.0+1.0;
     else return 2.0;
  }
  ...
  template <int dim>
  class InitialValuesW : public Function<dim>
  {
    public:
      InitialValuesW () : Function<dim>() {}
    ...
    template <int dim>
  double InitialValuesW<dim>::value (const Point<dim>  &p,
                                     const unsigned int component) const
  {
    Assert (component == 0, ExcInternalError());
//    if (p[1]*p[1] + (p[2]-0.003)*(p[2]-0.003)>0.7*0.7*0.001*0.001 )return 0.0;
...
//    else return 2.0;
  ...
  template <int dim>
  class BoundaryValuesW : public Function<dim>
  {
    public:
      BoundaryValuesW () : Function<dim>() {}
  ...
  template <int dim>
  double BoundaryValuesW<dim>::value (const Point<dim> &p,
                                      const unsigned int component) const
  {
    Assert (component == 0, ExcInternalError());
    return 0;
  ...
  class Material_Compressible_Neo_Hook_Three_Field
  {
    public:
      ...
      void update_material_data(const Tensor<2, dim> & F,
                                ...
                                const double J_tilde_in, const double mu,
                                                 const double nu, double c_density)
        {
          det_F = determinant(F);
          ...
          kappa = c_density*(2.0 * mu * (1.0 + nu)) / (3.0 * (1.0 - 2.0 * nu));
          c_1 = mu / 2.0 * c_density;
  ...
  class PointHistory
  {
    public:
      ...
      virtual ~PointHistory()
        {
          ...
          material = NULL;
				       ...
				       // values.
      void setup_lqp (const Parameters::AllParameters & parameters, double c_density)
        {
          material = new Material_Compressible_Neo_Hook_Three_Field<dim>(parameters.mu*c_density,
                                                                         parameters.nu);
          update_values(Tensor<2, dim>(), 0.0, 1.0, parameters.mu, parameters.nu, c_density);
        }
      ...
      void update_values (const Tensor<2, dim> & Grad_u_n,
                          ...
                          const double J_tilde, const double mu, const double nu, double c_density)
        {
          const Tensor<2, dim> F
            = (Tensor<2, dim>(StandardTensors<dim>::I) +
               Grad_u_n);
          material->update_material_data(F, p_tilde, J_tilde, mu, nu, c_density);
...
//*****************************************************************************************
      FE_Q<dim>            rd_fe;      
      ConstraintMatrix     rd_constraints;
      SparsityPattern      rd_sparsity_pattern;
      SparseMatrix<double> mass_matrix;
      SparseMatrix<double> laplace_matrix;
      SparseMatrix<double> matrix_u;
      SparseMatrix<double> matrix_v;
      SparseMatrix<double> matrix_w;

      Vector<double>       solution_u;
      Vector<double>       solution_v;
      Vector<double>       solution_w;
      Vector<double>       old_solution_u;
      Vector<double>       old_solution_v;
      Vector<double>       old_solution_w;
      Vector<double>       rd_system_rhs_u;
      Vector<double>       rd_system_rhs_v;
      Vector<double>       rd_system_rhs_w;
      const double theta;
      double time_step;
//*****************************************************************************************
//*****************************************************************************************
...
// we want. Have a look at its usage in step-20 for more information.
  ...
  void Solid<dim>::run()
  {
    make_grid();
    system_setup();
    std::cout<<"system_setup ok..."<<std::endl;
    {
      ConstraintMatrix constraints;
      constraints.close();
...
//**********************************************************************************
      VectorTools::project (rd_dof_handler, 
                           rd_constraints, 
                           QGauss<dim>(3),
                           InitialValuesU<dim>(),
                           old_solution_u);
...
//*************************************************************************************************
        for(int i=0;i<20;i++)run_reaction_diffusion();
//*************************************************************************************************
//*************************************************************************************************
//*************************************************************************************************
        solution_delta = 0.0;
...
// for comparison:
  ...
  {    
    const double inner_radius = 0.7,outer_radius = 1;
    GridGenerator::cylinder_shell (triangulation, 6, inner_radius, outer_radius);
    
    GridTools::scale(parameters.scale, triangulation);
    
    typename Triangulation<dim>::active_cell_iterator cell =
    triangulation.begin_active(), endc = triangulation.end();
    std::cout << "parameters.scale=" << parameters.scale << std::endl;
    ...
    for (; cell != endc; ++cell)
      for (unsigned int face = 0;face < GeometryInfo<dim>::faces_per_cell; ++face){
          	if (cell->face(face)->at_boundary())
        	  {
        	    ...
        	    else if (face_center[2] == 6*parameters.scale)
        	      cell->face(face)->set_boundary_indicator (1);
        	    else if (std::sqrt(face_center[0]*face_center[0] +
        			       face_center[1]*face_center[1]) < (inner_radius + outer_radius)*parameters.scale / 2
                           && face_center[2] > 0.5*parameters.scale && face_center[2] <5.5*parameters.scale)
        	      cell->face(face)->set_boundary_indicator (4);
//          	    else if (std::sqrt(face_center[0]*face_center[0] +
//        			       face_center[1]*face_center[1]) < (inner_radius + outer_radius)*parameters.scale / 2
//                           && face_center[2] > 2.5*parameters.scale && face_center[2] <3.5*parameters.scale)
//        	      cell->face(face)->set_boundary_indicator (4);
//          	    else if (std::sqrt(face_center[0]*face_center[0] +
//        			       face_center[1]*face_center[1]) < (inner_radius + outer_radius)*parameters.scale / 2
//                           && face_center[2] > 4.5*parameters.scale && face_center[2] <5.5*parameters.scale)
//        	      cell->face(face)->set_boundary_indicator (4);
      	        else if (std::sqrt(face_center[0]*face_center[0] +
      	                   face_center[1]*face_center[1]) < (inner_radius + outer_radius)*parameters.scale / 2)
    ...
    static const CylinderBoundary<dim> inner_cylinder_forced (inner_radius*parameters.scale, 2);
    static const CylinderBoundary<dim> inner_cylinder_unforced (inner_radius*parameters.scale, 2);
    static const CylinderBoundary<dim> outer_cylinder_fixed (outer_radius*parameters.scale, 2);
    static const CylinderBoundary<dim> outer_cylinder (outer_radius*parameters.scale, 2);
    triangulation.set_boundary (2, inner_cylinder_unforced);
    triangulation.set_boundary (3, outer_cylinder);
    triangulation.set_boundary (4, inner_cylinder_forced);
    triangulation.set_boundary (5, outer_cylinder_fixed);
    ...
    triangulation.refine_global(std::max (1U, 1U));//parameters.global_refinement));
...
// pressure and dilatation DOFs.
  ...
  void Solid<dim>::system_setup()
  {
    ...
    setup_qph();
    std::cout<<"setup for deformation ok..."<<std::endl;
//***********************************************************************************************
//*****************************section reaction-diffusion equation*******************************
//***********************************************************************************************
    rd_dof_handler.distribute_dofs (rd_fe);
    rd_sparsity_pattern.reinit (rd_dof_handler.n_dofs(),
                             rd_dof_handler.n_dofs(),
                             rd_dof_handler.max_couplings_between_dofs());
    DoFTools::make_sparsity_pattern (rd_dof_handler, rd_sparsity_pattern);
    rd_sparsity_pattern.compress();
    ...
    mass_matrix.reinit (rd_sparsity_pattern);
    laplace_matrix.reinit (rd_sparsity_pattern);
    matrix_u.reinit (rd_sparsity_pattern);
    matrix_v.reinit (rd_sparsity_pattern);
    matrix_w.reinit (rd_sparsity_pattern);

    MatrixCreator::create_mass_matrix (rd_dof_handler, QGauss<dim>(3),
                                       mass_matrix);
    MatrixCreator::create_laplace_matrix (rd_dof_handler, QGauss<dim>(3),
                                          laplace_matrix);
                                     ...
                                     // down below anyway.
    solution_u.reinit (rd_dof_handler.n_dofs());
    solution_v.reinit (rd_dof_handler.n_dofs());
    solution_w.reinit (rd_dof_handler.n_dofs());
    old_solution_u.reinit (rd_dof_handler.n_dofs());
    old_solution_v.reinit (rd_dof_handler.n_dofs());
    old_solution_w.reinit (rd_dof_handler.n_dofs());
    rd_system_rhs_u.reinit (rd_dof_handler.n_dofs());
    rd_system_rhs_v.reinit (rd_dof_handler.n_dofs());
    rd_system_rhs_w.reinit (rd_dof_handler.n_dofs());

    rd_constraints.close ();
//*************************************************************************************************
//*************************************************************************************************
//*************************************************************************************************
  ...
  template <int dim>
  void Solid<dim>::solve_u ()
  {
    SolverControl           solver_control (1000, 1e-8*rd_system_rhs_u.l2_norm());
    SolverCG<>              cg (solver_control);

    cg.solve (matrix_u, solution_u, rd_system_rhs_u,
  ...
  template <int dim>
  void Solid<dim>::solve_v ()
  {
    SolverControl          solver_control (1000, 1e-8*rd_system_rhs_v.l2_norm());
    SolverCG<>             cg (solver_control);
    cg.solve (matrix_v, solution_v, rd_system_rhs_v,
              PreconditionIdentity());
  ...
  template <int dim>
  void Solid<dim>::solve_w ()
  {
    SolverControl          solver_control (1000, 1e-8*rd_system_rhs_w.l2_norm());
    SolverCG<>             cg (solver_control);
    cg.solve (matrix_w, solution_w, rd_system_rhs_w,
              PreconditionIdentity());
    std::cout << "   w-equation: " << solver_control.last_step()
              << " CG iterations."
              << std::endl;
  }
...
// once the grid is refined to its finest level.
  ...
  void Solid<dim>::setup_qph()
  {
    ...
    for (typename Triangulation<dim>::active_cell_iterator cell =
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; ++q_point){
          //modify set_lqp() function to accept two parameters now            
          lqph[q_point].setup_lqp(parameters,1);
        }
    }
  }
...
// along to each QP storage object for processing.
                                              ...
                                              PerTaskData_UQPH & data)
  {
    ...
    typename DoFHandler<dim>::active_cell_iterator rd_cell = rd_dof_handler.begin_active(),
						 rd_endc = rd_dof_handler.end();
    typename DoFHandler<dim>::active_cell_iterator cell1 = dof_handler_ref.begin_active();
    while(cell1 != cell){
        cell1++;
        rd_cell++;
    }
...
//***************************************************************************************************************
//**************************this is to implement the variation of material properties point-by-point*************
//*************************************************************************************************************** 
    FEValues<dim> fe_values (rd_fe, qf_cell,//in step-8 here is "quadrature_formula"
    ...
    //cdensity.vector_value_list (fe_values.get_quadrature_points(), cde_values);
                                     ...
                                     // point data:
    fe_values.reinit(rd_cell);
                                              
    std::vector<Point<dim> > points = fe_values.get_quadrature_points();
    Assert (cde_values.size() == points.size(), 
 	    ExcDimensionMismatch (cde_values.size(), points.size()));   
    const unsigned int n_points = points.size();
        
    for (unsigned int p=0; p<n_points; ++p){
        //std::cout<<"p"<<p<<" "<<points[p]<<std::endl;
        
        Vector<double> TNF_value(1);
        VectorTools::point_value(rd_dof_handler, solution_u, points[p], TNF_value); 
        if(TNF_value(0)>0 && TNF_value(0)<1)cde_values[p](0) = 1.0-0.5*TNF_value(0);
        else if(TNF_value(0)>=1) cde_values[p](0) = 0.5;
        else cde_values[p](0) = 1;
    }
                //std::cout<<"cde="<<cde_values[p](0)<<std::endl;                                          
                                              
                                              
    double c_density;
    for (unsigned int q_point = 0; q_point < n_q_points; ++q_point){
          //std::cout<<"q"<<q_point<<std::endl;
          c_density = cde_values[q_point](0);             
          //lqph[q_point].setup_lqp(parameters,c_density);
          lqph[q_point].update_values(scratch.solution_grads_u_total[q_point],
                                  scratch.solution_values_p_total[q_point],
                                  scratch.solution_values_J_total[q_point],parameters.mu, parameters.nu, c_density);
    }
  }
...
// reset the error storage objects and print solver header.
  ...
  Solid<dim>::solve_nonlinear_timestep(BlockVector<double> & solution_delta)
  {
    ...
    for (; newton_iteration < parameters.max_iterations_NR;
         ++newton_iteration)
      {
        ...
        if (newton_iteration > 0 && error_update_norm.u <= 0.001)//parameters.tol_u
  ...
  template <int dim>
  void Solid<dim>::run_reaction_diffusion()
  {
...
//         time+=time_step, ++timestep_number)
    ...
    double//parameters for TNFalpha
        k_TNFN=2.97,
        k_TNFM=0.1,
        x_TNF10=0.079,
        x_TNF6=0.059,
...
//     Reaction = (k_TNFN*N+k_TNFM*M)*5*pow(value_v/x_TNF6,2.0)/(1+pow(value_v/x_TNF6,2.0))
//                                             /(1+pow(value_w/x_TNF10,2.0)) - k_TNF*value_u;
    int i;                                         
    tmp = old_solution_v;
    tmp /= x_TNF6;
    for(i=0;i<old_solution_v.size();i++)tmp[i] = tmp[i]*tmp[i];
    for(i=0;i<old_solution_v.size();i++)tmp2[i] = tmp[i]/(1+tmp[i]);
    
    tmp = old_solution_w;
    tmp /=x_TNF10;
    for(i=0;i<old_solution_v.size();i++)tmp[i] = tmp[i]*tmp[i];
    tmp.add(1.0);
    for(i=0;i<old_solution_v.size();i++)tmp2[i] = tmp2[i]/tmp[i];
    tmp2 *= (k_TNFN*N+k_TNFM*M)*5;
    ...
    rd_system_rhs_u.add (theta * time_step, forcing_terms);

    laplace_matrix.vmult (rd_system_rhs_u, solution_u);
    rd_system_rhs_u *= -0.05*0.001*0.001*theta * time_step;

    mass_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs_u += tmp;

    laplace_matrix.vmult (tmp, old_solution_u);
    rd_system_rhs_u.add (-0.05*0.001*0.001*time_step * (1-theta), tmp);

    rd_system_rhs_u += forcing_terms;

    {
        BoundaryValuesU<dim> boundary_values_u_function;
        boundary_values_u_function.set_time (time.current());

        std::map<unsigned int,double> boundary_values;
        VectorTools::interpolate_boundary_values (rd_dof_handler,
                                                0,
...
//*****************************solve for v*************************************************
    double//parameters for IL_6
        ...
        k6=0.7,
        S6=0.001;
//    Reaction = (k_6N*N+M)*(k_6M+k_6TNF*pow(value_u,2.0)/(pow(x_6TNF,2.0)+pow(value_u,2.0)))
//                           /(1+pow(value_w/x_610,2.0)) 
//                           + k6*(S6-value_v); 
    tmp = old_solution_u;
    for(i=0;i<old_solution_v.size();i++)tmp[i] = tmp[i]*tmp[i];
    for(i=0;i<old_solution_v.size();i++)tmp2[i] = tmp[i]/(x_6TNF*x_6TNF+tmp[i]);
    tmp *= k_6TNF;
    tmp.add(k_6M);
    tmp *= (k_6N*N+M);
    ...
    rd_system_rhs_v.add (theta * time_step, forcing_terms);

    laplace_matrix.vmult (rd_system_rhs_v, solution_v);
    rd_system_rhs_v *= -0.05*0.001*0.001*theta * time_step;

    mass_matrix.vmult (tmp, old_solution_v);
    rd_system_rhs_v += tmp;

    laplace_matrix.vmult (tmp, old_solution_v);
    rd_system_rhs_v.add (-0.05*0.001*0.001*time_step * (1-theta), tmp);

    rd_system_rhs_v += forcing_terms;

    {
        BoundaryValuesV<dim> boundary_values_v_function;
        boundary_values_v_function.set_time (time.current());
...
//**********************solve for w****************************************************
    double//parameters for IL10
        ...
        k10=0.35,
        S10=0.01;
//  Reaction = (k_10N*N+M)*(k_10M+k_10TNF*pow(value_u,4.0)/(pow(x_10TNF,4.0)+pow(value_u,4.0))
//                                   +k_106*pow(value_v,4.0)/(pow(x_106,4.0)+pow(value_v,4.0))) 
//                                   + k10*(S10-value_w);
    tmp = old_solution_u;
    for(i=0;i<old_solution_u.size();i++)tmp[i] = tmp[i]*old_solution_u[i];
    for(i=0;i<old_solution_u.size();i++)tmp[i] = tmp[i]*tmp[i];
    for(i=0;i<old_solution_v.size();i++)tmp2[i] = tmp[i]/(x_10TNF*x_10TNF*x_10TNF*x_10TNF+tmp[i]);
    tmp2 *= k_10TNF;
    
    tmp = old_solution_v;
    for(i=0;i<old_solution_v.size();i++)tmp[i] = tmp[i]*old_solution_v[i];
    for(i=0;i<old_solution_v.size();i++)tmp[i] = tmp[i]*tmp[i];
    for(i=0;i<old_solution_v.size();i++)tmp[i] = tmp[i]/(x_106*x_106*x_106*x_106+tmp[i]);
    tmp *= k_106;
    tmp2 += tmp;
    tmp2.add(k_10M);
    tmp2 *= k_10N*N+M;
    ...
    rd_system_rhs_w.add (theta * time_step, forcing_terms);

    laplace_matrix.vmult (rd_system_rhs_w, solution_w);
    rd_system_rhs_w *= -0.05*0.001*0.001*theta * time_step;

    mass_matrix.vmult (tmp, old_solution_w);
    rd_system_rhs_w += tmp;

    laplace_matrix.vmult (tmp, old_solution_w);
    rd_system_rhs_w.add (-0.05*0.001*0.001*time_step * (1-theta), tmp);

    rd_system_rhs_w += forcing_terms;

    {
        BoundaryValuesV<dim> boundary_values_w_function;
        boundary_values_w_function.set_time (time.current());

        std::map<unsigned int,double> boundary_values;
        VectorTools::interpolate_boundary_values (rd_dof_handler,
                                                0,
                                                boundary_values_w_function,
                                                boundary_values);
        matrix_w.copy_from (mass_matrix);
        MatrixTools::apply_boundary_values (boundary_values,
                                          matrix_w,
                                          solution_w,
                                          rd_system_rhs_w);
                                           ...
                                           PerTaskData_RHS & data)
  {
    ...
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
      ...
      if (cell->face(face)->at_boundary() == true
          && cell->face(face)->boundary_indicator() == 4)
        {
          ...
          for (unsigned int f_q_point = 0; f_q_point < n_q_points_f;
               ++f_q_point)
            {
              ...
              static const double  p0        = 2.0/(parameters.scale * parameters.scale);
              const double         time_ramp = (time.current() / time.end());
              const double         pressure  = p0 * parameters.p_p0 * 0.1;//time_ramp;
...
//********************************************************************************************
//*******************readtion diffusion equation**********************************************
//********************************************************************************************
    DataOut<dim> rd_data_out;

    rd_data_out.attach_dof_handler (rd_dof_handler);
    rd_data_out.add_data_vector (solution_u, "U");

    rd_data_out.build_patches ();

    const std::string rd_filename = "rd_solution-" +
                                 Utilities::int_to_string (time.get_timestep(), 3) +
                                 ".vtk";
    std::ofstream rd_output (rd_filename.c_str());
    rd_data_out.write_vtk (rd_output);
  }
