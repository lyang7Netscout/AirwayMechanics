...
//    std::cout<<"b_indicator2 ="<<b_i(1);
    ...
    static const HyperBallBoundary<dim> boundary_description(Point<dim>(0.0,0.0,0.0),parameters.scale);
    ...
    for (; cell != endc; ++cell)
      for (unsigned int face = 0;
           face < GeometryInfo<dim>::faces_per_cell; ++face){
        if (cell->face(face)->at_boundary() == true
            && cell->face(face)->center()[2] > 0.85 * parameters.scale)
            cell->face(face)->set_boundary_indicator(1);
        if (cell->face(face)->at_boundary() == true
            && cell->face(face)->center()[2] < -0.75 * parameters.scale)
            cell->face(face)->set_boundary_indicator(2);
      } 
    static const HyperBallBoundary<dim> boundary_description1(Point<dim>(0.0,0.0,0.0),parameters.scale);
    triangulation.set_boundary(1, boundary_description1);
    static const HyperBallBoundary<dim> boundary_description2(Point<dim>(0.0,0.0,0.0),parameters.scale);
