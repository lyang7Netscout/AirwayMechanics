    ...
    {	
        ...
        for (; cell != endc; ++cell){
            for(unsigned int v=0; v< GeometryInfo<3>::vertices_per_cell; ++v){
                const double distance_from_centerline = 
                                     sqrt(cell->vertex(v)[0]*cell->vertex(v)[0] 
                                        + cell->vertex(v)[1]*cell->vertex(v)[1]);
                std::cout<<"distance="<<distance_from_centerline<<std::endl;
                if(std::fabs(distance_from_centerline - inner_radius*0.001) < 1e-10){
        ...
        std::cout<<"refined "<<step<<" times"<<std::endl;
    }
